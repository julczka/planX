...
export default {
  ...
  computed: {
    transform() {
      ...
      const movementX =
        ((relativeX - shape.width / 2) / (shape.width / 2)) * parallaxStrength;
      const movementY =
        ((relativeY - shape.height / 2) / (shape.height / 2)) *
        parallaxStrength;

      let transform;
      if (this.type === "translation") {
        transform = `translate3d(${this.limitY ? 0 : -movementX}px, ${
          this.limitX ? 0 : -movementY
        }px, 0)`;
        ...
        transform = `rotateX(${this.limitX ? 0 : -movementY}deg) rotateY(${
          this.limitY ? 0 : movementX
        }deg) translate3d(0,0,${this.parallaxStrength * 2}px)`;
      }
